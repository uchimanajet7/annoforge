...
- ツール: 選択/矩形/直線/多角形/平行四辺形/円。
  - 既存の図形を移動・編集できるのは選択ツール中だけです。作図ツール中は、既存図形と重なる位置から新しい図形を作成できます。
...
- 選択: キャンバスでは最前面の図形を選択します。重なって隠れた図形はアノテーション一覧から選択でき、選択中だけ編集用に前面表示されます。選択解除後の表示順と JSON の順序は変わりません。
- 表示操作: 右下の＋/−/リセット、選択ツール中は背景ドラッグでパン。タッチはピンチズーム。
- 色: パレットまたはカラーピッカー。未選択時は次に作成する形状へ適用し、確定済み形状を1件選択中はその形状だけの輪郭線を変更します。重なった形状は一覧から対象を選べます。既定太さは元画像座標で 5px です。
- JSON/入出力: 常時更新。コピー/ダウンロード/インポートに対応します。貼付/ファイル/ドロップで取り込めます。
- 保存: 「注釈付き画像を保存」で、表示のパンやズームに左右されない元画像寸法・画像範囲のPNGを出力します。JSONとPNGのダウンロード要求後は、ブラウザーのダウンロード一覧で保存完了を確認してください。

## WebMCP（ChatGPT Site tools）

AnnoForge は提案中の [WebMCP API](https://webmachinelearning.github.io/webmcp/) を利用し、ChatGPT Site tools などの対応環境で、人とエージェントが同じページと作業状態を共有できるようにします。エージェントは画像を開き、見た目と JSON を確認し、アノテーションを反映し、完成 PNG の Data URL をツール呼び出し元へ返せます。会話への画像表示またはファイル添付は対応クライアント側の機能です。人は同じキャンバス上で途中確認、編集、保存を行えます。

- `open_image_from_url`: HTTPS画像（クロスオリジンの場合は配信元のCORS許可が必要）、または同一オリジンのloopback HTTP画像を開きます。
- `open_image_from_data_url`: 呼び出し側から渡されたPNG/JPEG/WebPのData URLを開きます。会話の添付ファイルを直接読み取る機能ではありません。
- `get_annotations`: 現在の作業版、全アノテーション、元画像の寸法を読み取ります。
- `get_image_preview`: 元画像の縦横比を保ち、表示状態や選択表示を含めない注釈付き縮小PNGを返します。元画像より拡大しません。
- `replace_annotations`: 読み取った作業版が最新の場合だけ、整数ピクセル座標を含む全項目の検証後に、全アノテーションを指定された `draw` へ置換します。
- `start_annotations_json_download`: アノテーションJSONを生成し、ブラウザーへダウンロードを要求します。結果は要求送信済み・保存完了未確認として返します。
- `export_annotated_image`: 元画像と同じ寸法・範囲の完成PNGを Data URL として返すか、ブラウザーへダウンロードを要求します。Data URL の会話表示とブラウザーでの保存完了は、それぞれクライアントとブラウザーで確認します。
- フォールバック: AnnoForge は画面操作へ自動で切り替えず、独自のフォールバック指示や操作許可も返しません。Site toolの不在・実行エラー、または結果後の保存・会話提示を確認できない場合、呼び出し側は利用者の依頼と対応クライアントの通常の権限・確認手順に従って既存UIやファイル機能へ切り替えます。切替後の操作はWebMCPではありません。
