...
WebMCP 対応環境では、人とエージェントが同じ AnnoForge ページと作業状態を共有できます。エージェントは画像を開き、見た目と JSON を確認し、アノテーションを反映し、完成 PNG の Data URL をツール呼び出し元へ返せます。会話への画像表示またはファイル添付は対応クライアント側の機能です。人は同じキャンバス上で途中確認、編集、保存を行えます。
...
- フォールバック: WebMCP ツールの結果は、通常のブラウザー操作を許可しません。ツールがエラーになった場合、利用者が許可した範囲内でだけ、呼び出し側が既存の画面上の保存操作を使用できます。AnnoForge は自動で切り替えません。
