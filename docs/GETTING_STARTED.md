...
- `export_annotated_image` の `data_url` で、元画像と同じ寸法・範囲の完成PNGデータをツール呼び出し元へ返します。会話への画像表示またはファイル添付は対応クライアント側で行います。人向けファイル保存が必要な場合だけダウンロード経路を使います。
- JSONまたはPNGのダウンロード結果が `outcome: "download_requested"` の場合、AnnoForgeが確認できたのはブラウザーへの要求送信までです。`completionVerified: false` のため、保存完了はブラウザーのダウンロード一覧で確認します。
- WebMCPツールがエラーになった場合、ツール結果自体は通常のブラウザー操作を許可しません。利用者が通常操作を許可しており、対応クライアントが実行できる場合だけ、既存の画面上の保存ボタンを使用します。AnnoForgeは自動でフォールバックしません。
