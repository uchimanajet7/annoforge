...
- スコープ: WebUI の単一ページにおける画像ファイル・画像URLの読み込み、描画・編集・一覧・JSON出力・コピー機能。
...
### 3.1.1 画像の読み込み
- ファイル選択: ヘッダの「画像を選択」から画像ファイルを選ぶ。
- ドラッグ＆ドロップ: キャンバスへ画像ファイルをドロップする。ドロップ中は枠をハイライト表示する。
- URL入力: 「画像を選択」の隣の「URLから画像を開く」から専用ダイアログを開く。「画像URL」「開く」「キャンセル」と、必要な入力条件、読込み中・エラーの表示を提供する。新しいサーバーやWebMCP対応を必要としない。
- URLの許可・取得・検証は人向けUIとWebMCPで共通化する。資格情報を含まないHTTPSの絶対URLを許可し、クロスオリジンでは配信元のCORS許可を必要とする。loopbackのHTTPページでは同一オリジンのHTTP画像も許可する。PNG/JPEG/WebP、12 MiB以下とし、HTTP応答・リダイレクト先・MIME・サイズ・デコードを検証する。この上限は製品設定であり、ローカルファイルに同じ形式・容量制限を追加しない。
- 注釈または作図中の図形がある場合だけ、置換による消去と元に戻せないことを確認する。URL入力では同じダイアログ内に警告と「注釈を破棄して開く」を表示し、確認ダイアログを重ねない。ファイル選択とドロップでも同じ消去条件で確認する。初回読込み・作業のない画像置換には確認を追加しない。
- ダイアログはHTMLの `dialog` と `showModal()` を使用する。フォーカス移動・閉じた後の復帰・背景の操作抑制・Escapeでの取消しを利用する。入力欄やダイアログ内のEnterをキャンバスの多角形確定へ伝播させない。
- 読込み開始・取消し・結果反映は、ファイル選択、ドロップ、UIのURL入力、WebMCPのURL/Data URL入力で共通化する。入力検証と必要な確認を終えて読込みを開始したときだけ、前の読込みを無効化する。ダイアログを開く、入力する、ファイル選択を取り消すだけでは無効化しない。
- 画像の取得・デコード成功後、処理が最新か、取消されていないか、開始時から作業内容が変わっていないかを再確認してから、画像・注釈・表示・revisionを反映する。遅れて完了した旧処理は新しい画像を上書きしない。失敗・取消し・競合では現在の画像と作業を保持する。確認中の作業変更は確認内容を更新し、読込み中の作業変更は適用を中止して再操作を案内する。
- 人向け読込みは取消し可能とする。URLではダイアログ内、ファイルでは読込み中だけ画像入力の隣に状態と取消しを表示する。完了時には読込み中表示を解除し、一時URLと中断リスナーを解放する。
- 初期表示: 読み込み後はステージ内に収まる比率で表示し、画像中心がステージ中心となる位置に配置する。拡大は行わない。

設計根拠（2026-09-07確認）:
- [VIAの画像追加](https://www.robots.ox.ac.uk/~vgg/software/via/docs/add_images_to_project.html)はファイルとURLの入力を提供する。本製品では単一画像を扱うため、既存の画像入力グループにURL入力を加える。
- [W3C H102](https://www.w3.org/WAI/WCAG21/Techniques/html/H102)の標準dialogによるフォーカス管理と、[W3C G168](https://www.w3.org/WAI/WCAG21/Techniques/general/G168)の取消不能な操作の結果を伝える確認を採用する。特定の配置や日本語ラベルを規格の必須事項とは扱わない。
- [MDNのFetch取消し](https://developer.mozilla.org/en-US/docs/Web/API/Fetch_API/Using_Fetch#canceling_a_request)を取得処理へ適用し、デコード後の旧結果にも反映前の作業版確認を行う。[ChromeのWebMCP指針](https://developer.chrome.com/docs/ai/webmcp/best-practices)に沿って入力検証とUI反映を実装側で揃える。

### 3.2 ツール/操作
- 選択ツール: 確定済み形状の選択と移動を行う唯一のツール。矩形と円は Konva Transformer で拡大縮小と回転に対応。直線・多角形・平行四辺形は頂点アンカーで形状を編集し、回転用のTransformerは使わない。
- 作図ツール: 矩形・直線・多角形・平行四辺形・円の各作図ツールでは、確定済み形状をドラッグ移動しない。既存形状と重なる位置からも新しい形状を作成できる。
- 矩形: ドラッグで作成。選択ツールへ切り替えた後、Transformerのハンドルで編集できる。
- 直線: クリックしてドラッグで終点を決め、離して確定。選択ツールでは端点をドラッグできる。ヒット判定は `hitStrokeWidth` を使用。
- 端点スナップ: 直線作図時のみ、既存頂点の近傍で端点を自動スナップし、マーカーを表示する。
- 多角形: 連続クリックで頂点追加、ダブルクリック/Enterで確定。選択ツールでは各頂点（アンカー）をドラッグして編集できる。
- 平行四辺形: 3点指定。P1,P2 で基底ベクトル、P3 でオフセット。4点目は自動補完します。P4 = P3 + (P2 - P1)。描画・保存時の頂点順は [P1, P2, P4, P3] とします。辺の並びが a, b, -a, -b となり、対向辺が平行です。選択ツールでは頂点を編集できる。
- 円: 中心をクリックで開始し、ドラッグで半径を可視化してマウスアップで確定。選択ツールへ切り替えた後はドラッグで移動し、Transformerで等倍スケーリングします。transformend で scale を半径に正規化します。
...
- 確定済み形状を1件選択している場合、色を選ぶと、その形状の輪郭線だけを変更する。対象は矩形・直線・多角形・平行四辺形・円の全5種類とし、塗り、形状種別、座標、太さ、回転、表示順、JSON順序、ID、選択状態は変更しない。
- 形状を選択していない場合、選んだ色は次に作成する形状へ適用する。
- 形状を選択しただけでは次回作図色を変更しない。選択中に利用者が色を選んだ場合は、選択形状を変更すると同時に、その色を次回作図色として記憶する。
- 選択形状と同じ色を再度選んだ場合、アノテーションの作業版は変更せず、その色を次回作図色として記憶する。
- 色変更後は、表示中のKonvaノード、一覧の色スウォッチ、JSON、注釈付きPNG、WebMCPの取得・プレビュー・出力へ同じ値を反映し、アノテーションが実際に変化した場合だけ `workspaceRevision` を1増加する。
- 重なった形状は一覧から対象を一意に選択してから色を変更できる。複数選択、一括変更、塗り色編集、ラベル分類、太さ編集、Undo/Redoは対象外とする。
- 現在の適用先と色は、カラーピッカーのアクセシブル名とパレットの選択状態へ反映する。パレットの選択状態は色だけに依存せず、外枠と `aria-pressed` でも示す。

設計根拠（2026-09-04確認）:
- tldrawは選択形状のスタイルと次に作成する形状のスタイルを区別し、選択中のスタイル変更を次回作図にも記憶する。この操作モデルを単一選択へ限定して採用する: https://tldraw.dev/sdk-features/styles
- CVATは重なりを含む個別オブジェクトを一覧から特定でき、インスタンス単位の色変更を提供する。重なった対象を一覧から一意に選ぶ導線の根拠とする: https://docs.cvat.ai/docs/annotation/annotation-editor/objects-sidebar/
- Konvaは既存Shapeの `stroke` を直接更新できるため、図形の再作成やJSONスキーマ変更を必要としない: https://konvajs.org/api/Konva.Shape.html
- 色だけを状態表示に使わないというWCAGの要件と、トグルボタンの `aria-pressed` パターンに従う: https://www.w3.org/WAI/WCAG22/Understanding/use-of-color / https://www.w3.org/WAI/ARIA/apg/patterns/button/
- カラーピッカーは選択中の連続変更を反映するため、`input` イベントを使用する: https://developer.mozilla.org/en-US/docs/Web/HTML/Reference/Elements/input/color

### 3.3 当たり判定/編集性
- 直線: `hitStrokeWidth = max(8, thickness)` で掴みやすさ確保。
- 頂点アンカー: 半径6pxの可視ハンドル。ドラッグで点移動。多角形のみ、ダブルクリックで点削除。
- Transformer: 矩形と円の単一選択時に表示。
- 重なり: キャンバス上のクリックでは最前面の形状を選択する。一覧では表示順や重なりにかかわらず、対応する形状を一意に選択できる。
- 選択中の形状は編集できるよう表示上だけ一時的に最前面へ移し、Transformer または頂点アンカーをその上に表示する。別の形状の選択時または選択解除時に、表示順を `shapes[]` の順へ戻す。`shapes[]` と JSON の順序は変更しない。
...
- 表示用ステージのパン、ズーム、フィット倍率、位置、ウィンドウ寸法は出力結果へ影響させない。
- 作図中のドラフト、Transformer、頂点アンカー、スナップマーカー、選択枠、選択・ドラッグによる編集用表示は含めない。
- 矩形と円の確定済み表示ノードに現在の回転がある場合は PNG へ反映する。回転角を JSON に含めない既存仕様は維持し、回転の永続化は本修正では追加しない。
- 人向けの「注釈付き画像を保存」と WebMCP の最終 PNG・プレビューは同じ出力専用レンダラーを使う。人向け UI、PNG 形式、既存のファイル名規則は維持する。
- ブラウザーが画像寸法またはエンコードを処理できない場合は、不完全な成功として扱わず明示的なエラーにする。製品独自の固定寸法上限は追加しない。

### 3.4.2 ファイル出力の状態契約
- JSON と注釈付き PNG は、成果物の生成、ブラウザーへのダウンロード要求、ブラウザーによる保存完了、会話への表示または添付を別の状態として扱う。
- AnnoForge がダウンロード経路で保証する成功範囲は、対象 revision の成果物を生成し、ブラウザーへダウンロード要求を送信したところまでとする。通常のページ JavaScript から保存完了、キャンセル、保存先を確認できないため、完了とは表示しない。
- ダウンロード要求を送信した結果には、要求送信済み、完了未確認、ファイル名、MIME type、バイト数、対象 revision、アノテーション数を含める。PNG では幅と高さも含める。
- Data URL 経路は PNG データを呼び出し元へ返すところまでを保証する。呼び出し元がそのデータを会話へ画像またはファイルとして表示・添付したかどうかは、AnnoForge の保証範囲外とする。
- 人向け UI と WebMCP は、JSON または PNG の成果物生成とブラウザーへのダウンロード要求処理を共有する。片方だけ異なる生成処理または成功判定を持たせない。
- ブラウザーへの要求送信に失敗した場合はエラーとして扱う。要求送信後の Blob URL は直後に破棄せず、ダウンロード開始処理が参照できる時間を確保してから解放する。
- 人向け UI の既存ボタン、ラベル、操作手順は維持する。要求送信後は、保存完了ではなく、ブラウザーのダウンロード一覧で完了状態を確認するよう通知する。

### 3.5 一覧・操作
- 一覧項目: 種別名、色スウォッチ、矩形/直線/円は主要座標/寸法、ポリゴン/平行四辺形は頂点数、削除ボタン。
- 選択: 一覧項目の選択操作で選択ツールへ切り替え、対応する形状を一意に選択する。選択状態は一覧上にも継続表示し、選択操作はキーボードでも実行できる。
- 削除: 選択操作とは独立した削除ボタンで行う。選択中の形状を削除した場合は選択状態も解除する。
- 全てクリア: 形状をすべて削除。
...
- 画像出力:
  - 表示用ステージを直接画像化せず、元画像と `shapes[]` から元画像境界の出力専用シーンを生成する。
  - 人向け保存は PNG Blob、WebMCP の画像返却は必要な経路だけ PNG Data URL を生成する。
  - JSON対象外である矩形・円の回転だけは、対応する確定済み表示ノードの現在値を出力専用シーンへ引き継ぐ。
- ファイル出力:
  - JSON と PNG は、Blob とファイルメタデータを生成する共通の成果物境界を持つ。
  - 人向け UI と WebMCP のダウンロード経路は、同じブラウザーダウンロード要求処理を使う。
  - ダウンロード要求の結果は送信済みと完了未確認を明示し、ブラウザー保存完了や会話添付を推測しない。
  - エージェント向けの直接受信経路では、同じrevisionのPNG・JSON Blobを1組準備し、SHA-256付きメタデータと、指定範囲のbase64バイト列をWebMCPで返す。自動ダウンロードや追加バックエンドは使用しない。
  - 準備済み出力は編集・新しい準備の成功・明示解放・ページ終了で無効になる。受信中のrevisionや出力IDの変化を拒否する。人向けUIに出力IDや受信手順を追加しない。
  - 同梱のクライアント受信モジュールは、許可された保存先へPNG・JSONを新規保存して整合性を検証する。会話への提示には呼び出し側が返された実ファイルを添付する。公開契約・実行例は [WEBMCP.md](./WEBMCP.md) を参照する。
- 編集:
  - 矩形と円はTransformerで編集。直線・多角形・平行四辺形は頂点アンカーで編集。
  - 色コントロールの表示値は、単一選択中は選択形状の `colorHex`、未選択時は次回作図色とする。形状の選択操作だけでは次回作図色を書き換えない。
  - 利用者が色を選んだ時は、次回作図色を更新し、単一選択中で色が異なる場合だけモデルの `colorHex` と対応するKonvaノードの `stroke` を更新して `workspaceRevision` を進める。
  - 確定済み形状の `draggable` は選択ツール中だけ有効にし、作図ツール中と作図中のドラフトでは無効にする。
  - ドラッグ終了時は形状ごとの座標表現を維持する。直線・多角形・平行四辺形はモデルへ移動量を統合してノード位置を `(0,0)` に正規化し、矩形・円はモデルの `x/y` をノード位置として保持する。
  - 一覧またはキャンバスからの選択は作業版の変更ではないため `workspaceRevision` を進めない。選択中だけ表示ノードを前面化し、確定モデルと JSON の並びは維持する。
...
- 一覧選択: 一覧項目をクリックまたはキーボードで実行すると選択ツールへ切り替わり、対応形状を選択して編集表示を前面へ出す。
- 選択解除: 選択ツールで背景を押すか、作図ツールへ切り替える。選択中の一時的な前面表示を解除し、確定モデル順の表示へ戻す。
- 削除: 一覧の削除ボタンで削除する。選択中の形状を削除した場合は選択も解除する。
...
## 8. アクセシビリティ/国際化
- キーボード操作: 多角形の確定は Enter に対応する。一覧の形状選択と削除はネイティブボタンで提供し、キーボードフォーカスを視認できるようにする。
- 文言: 日本語。
...
## 10. 受け入れ基準
- WebMCP非対応の通常ブラウザーでもURLから画像を開き、編集後に元画像寸法のPNGとJSONを保存できる。
- UIとWebMCPの両方向で同じ画像・注釈・revisionが見え、直接受信によるPNG/JSON出力も利用できる。
- 不正URL、HTTP/CORS/形式/容量/デコードの失敗、取消し、連続読込み、読込み中の編集で、以前の作業を消去したり旧結果で上書きしたりしない。
- URL入力・消去確認のキーボード操作、デスクトップと狭幅表示、既存のファイル選択・ドロップを確認する。
- 画像上で矩形/直線/多角形/平行四辺形/円が作成できる。
- 形状の基本編集ができる。矩形と円は移動と拡大縮小と回転に対応し、直線・多角形・平行四辺形は移動と頂点編集に対応する。
- 矩形または円を移動してドラッグを終了しても、表示位置と JSON の座標が一致し、原点へ移動しない。
- 作図ツール中は既存形状が移動せず、既存形状と重なる位置から新しい形状を作成できる。
- 一覧に各図形が反映され、重なって隠れた形状も一覧から一意に選択して編集でき、独立した削除ボタンで削除できる。
- 一覧またはキャンバスで選択した形状は編集表示だけ一時的に前面へ出るが、選択解除後の表示順、`shapes[]`、JSON の順序は変化しない。
- 確定済み形状を1件選択して色を選ぶと、その形状だけの輪郭線、一覧、JSON、PNG、WebMCP出力が同じ色へ更新され、形状の幾何情報と順序は変化しない。
- 形状を選択していない状態で選んだ色は次回作図だけに適用され、形状の選択操作だけでは次回作図色は変化しない。
- 選択形状と同じ色の再選択では `workspaceRevision` が変化せず、異なる色への変更では1増加する。
- JSONが常時更新され、クリップボードコピーできる。
- 直線の掴みやすさが担保されている（`hitStrokeWidth`）。
- 注釈付き PNG は元画像と同じピクセル寸法・画像境界で、表示のパン、ズーム、ステージ寸法に依存せず生成される。
- 注釈付き PNG には確定済み注釈だけが含まれ、ドラフト、選択表示、ガイド、一時的な強調表示は含まれない。
- 人向け UI と WebMCP のダウンロード経路は、JSONとPNGの成果物生成およびブラウザーへの要求処理を共有する。
- ブラウザーへダウンロード要求を送信した結果は、要求送信済みと保存完了未確認を区別し、ファイルの検証に必要なメタデータを返す。
- PNG Data URL の返却はツール呼び出し元へのデータ返却として扱い、会話への表示または添付完了とは扱わない。
- WebMCPの直接受信でPNGとJSONが同じrevisionの実ファイルとして取得でき、保存後のバイト数・SHA-256・PNG寸法・JSON注釈数が準備結果と一致する。PNGの会話提示はツールの文字列結果ではなく実ファイルの画像表示で確認する。
- 受信中の出力無効化や破損は成功扱いせず、この受信処理が作成した不完全ファイルだけを後処理する。再実行時も既存ファイルを上書きしない。

## 11. 既存からの変更点
- Canvas素描ロジックをKonva.jsベースに置換。
- JSONスキーマの拡張（`line`/`polygon`/`parallelogram`）。
- UIにツール選択セクションを追加。
- 注釈付き PNG の生成元を表示用ステージから元画像座標の確定モデルへ変更し、表示状態に依存しない元画像寸法の出力へ修正。
- JSONとPNGの保存処理を共通の成果物生成とブラウザーダウンロード要求へ分離し、要求送信と保存完了を区別する結果契約へ変更。
- WebMCP の PNG Data URL 返却とクライアントによる会話表示・添付を別の状態として明確化。
- WebMCPで準備済みPNG・JSONの分割読取りと解放を提供し、クライアントの受信モジュールで実ファイルの保存・検証から会話添付につなぐ経路を追加。
- 確定済み形状の移動を選択ツールだけに限定し、ドラッグ終了時の形状別座標正規化を明確化。
- 一覧クリックの一時的な線幅強調を、重なった形状も一意に指定できる継続選択と編集用の一時前面表示へ変更。
- 色コントロールを新規作図専用から、単一選択中はその形状、未選択時は次回作図へ適用する共通操作へ変更。個別色変更は既存の `color` フィールドと輪郭線表示を更新し、新しいデータ形式や一括編集UIは追加しない。
